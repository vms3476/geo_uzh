...
f = linspace(1,numel(features),numel(features));
f = f .* features;

for n = 1:numel(features)
    
    if f(n) % if the features vectors has a 1 for this feature #
        ...
        if n == 1
            ...
            % when LASread used to read the data, the return number field has
            % a different name
            if ~isfield(las,'rnnr')
                las.rnnr = las.return_number;
                las.int = las.intensity;
            end
        ...
        if n == 3
            ...
            clear singleEchoesDen totalEchoesDen singleEchoFraction 
        end
