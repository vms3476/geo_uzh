function [optfeat,LOG]=bootstrap_feature_validation_loop(options,optfeat,opts)
    ...
    for j=1:nbj
        for k=1:nbk
            for l=1:nbl
                ...
                if opts.verbose & mod(ite,5)==0

                    fprintf( ' %2.1f%%\n', 100*ite/(nbi*nbj*nbk*nbl))
                end
                ...
                for m=1:opts.nbloop
                    %feature extraction
                    [X2,Y2,X2val,Y2val]=opts.get_data(options2,m);

                    % learning
                    model=opts.learning(X2,Y2,options);
...
eval(['optfeat.' opts.params{1} '=' num2str(opts.values{1}(i)) ';']);
if nbparams>1,eval(['optfeat.' opts.params{2} '=' num2str(opts.values{2}(j)) ';']); end
if nbparams>2,eval(['optfeat.' opts.params{3} '=' num2str(opts.values{3}(k)) ';']); end
if nbparams>3,eval(['optfeat.' opts.params{4} '=' num2str(opts.values{4}(l)) ';']); end
