function i=enviwrite(image,fname,hdrstr);
%  function i=enviwrite(image,fname,hdrstr);
% enviwrite          	- write ENVI image from MATLAB array (V. Guissard, Apr 29 2004)
% added hdrstr variable, which may contain map-info, Felix Morsdorf, June 2007
...
% hdrstr [cell]         : cell array conating additional header lines (e.g. map-info);  
%
% OUTPUT :
%
...
wfid = fopen(fname,'w','l');
if wfid == -1
    i=-1;
end
disp([('Writing ENVI image ...')]);
%fwrite(wfid,fliplr(img'),cl);
fwrite(wfid,img,cl);
fclose(wfid);
